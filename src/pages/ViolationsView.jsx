...
import { AlertOctagon, ShieldAlert, Plus, Users, UserX, CheckCircle, AlertTriangle, FileText } from 'lucide-react';
...
export default function ViolationsView() {
  ...
  const loadData = () => {
    fetch('/api/violations').then(r => r.json()).then(setViolations).catch(() => {});
    fetch('/api/master/students').then(r => r.json()).then(data => {
      setStudents(data);
  ...
  const handleAddViolation = (e) => {
    e.preventDefault();
    if (!violationName) return showToast('Nama pelanggaran harus diisi', 'error');
                ...
                >
                  {students.map((s) => (
                    <option key={s.id} value={s.id}>
                      {s.name} - {s.class_name || 'X MIPA'} ({s.nisn})
                    </option>
                  ))}
                </select>
              </div>
