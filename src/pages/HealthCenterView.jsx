...
export default function HealthCenterView() {
  ...
  // Nama siswa hanya diisi otomatis untuk akun siswa; akun orang tua mengisi kolom wali; akun staf mengisi manual.
  const buildPermissionForm = () => ({
    student_id: isStudentSide ? (currentUser?.related_student_id || null) : null,
    student_name: currentRole === 'siswa' ? (currentUser?.name || '') : '',
    class_name: '',
    permission_date: todayLocal(),
    permission_type: 'izin',
    reason: '',
    parent_name: currentRole === 'ortu' ? (currentUser?.name || '') : '',
    parent_phone: currentRole === 'ortu' ? (currentUser?.phone || '') : '',
  });
  ...
  const openVisitForm = () => { setVisitForm(buildVisitForm()); setShowVisitForm(true); };
  const openPermissionForm = () => { setPermissionForm(buildPermissionForm()); setShowPermissionForm(true); };
  ...
  const createPermission = async (event) => {
    ...
    if (!payload.student_name || !payload.reason) { showToast('Nama siswa dan alasan izin wajib diisi.', 'error'); return; }
    setSaving(true);
    try {
  ...
  return (
    <div className="space-y-7 pb-12">
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
        ...
        <div className="flex flex-wrap gap-2 border-t border-slate-100 bg-slate-50 px-6 py-3 sm:px-8"><button onClick={openPermissionForm} className="inline-flex items-center gap-2 rounded-lg bg-[#002147] px-3.5 py-2 text-xs font-bold text-white hover:bg-[#0b315f]"><FileHeart className="h-3.5 w-3.5 text-[#f4a024]" /> Ajukan izin</button>{canManage && <button onClick={openVisitForm} className="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3.5 py-2 text-xs font-bold text-[#002147] hover:border-[#002147]"><ClipboardPlus className="h-3.5 w-3.5" /> Catat kunjungan UKS</button>}</div>
      </section>
